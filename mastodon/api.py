...
def obtain_token(site, request, code):
    ...
    redirect_uri = (
        settings.APP_WEBSITE + reverse("users:OAuth2_login")
    )
    payload = {
...
def share_topic(topic):
    ...
    visibility = TootVisibilityEnum.PUBLIC
    response = post_toot(
        user.mastodon_site, content, visibility, user.mastodon_token, False
    )
    if response and response.status_code in [200, 201]:
        j = response.json()
        if "url" in j:
            topic.shared_link = j["url"]
        if topic.shared_link:
            topic.save(update_fields=["shared_link"])
        return True
    else:
        return False
...
def share_comment(comment):
    ...
    if response and response.status_code in [200, 201]:
        j = response.json()
        if "url" in j:
            comment.shared_link = j["url"]
        if comment.shared_link:
            comment.save(update_fields=["shared_link"])
        return True
    else:
        return False
